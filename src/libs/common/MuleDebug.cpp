...
void InstallMuleExceptionHandler() { std::set_terminate(OnUnhandledException); }
...
void init_backtrace_info() {
  ...
  s_have_backtrace_symbols =
      (get_backtrace_symbols(s_abfd, &s_symbol_list) > 0);
}
...
void get_file_line_info(bfd *abfd, asection *section, void *_address) {
  ...
  s_found =
      bfd_find_nearest_line(abfd, section, s_symbol_list, address - vma,
...
static wxString demangle(const wxString &function) {
  ...
  if (function.Mid(0, 2) == wxT("_Z")) {
    int status;
    char *demangled =
        abi::__cxa_demangle(function.mb_str(), NULL, NULL, &status);
...
wxString get_backtrace(unsigned n) {
  ...
  for (int i = 0; i < num_entries; ++i) {
    ...
    if (s_found) {
      ...
      out.Insert(CFormat(wxT("%s:%u")) %
                     wxString(wxConvCurrent->cMB2WX(s_file_name)) %
                     s_line_number,
                 i * 2 + 1);
    } else {
      out.Insert(wxT("??"), i * 2);
  ...
  if (wxThread::IsMain()) {
    wxString command;
    command << wxT("addr2line -C -f -s -e /proc/") << getpid() << wxT("/exe ")
            << AllAddresses;
...
// Other code (stack frame list and demangle related) has been modified from
// code with license:
...
public:
  stack_frame(const void *f_instruction, const std::string &f_function)
      : frame_instruction(f_instruction), frame_function(f_function){};
    ...
    while (fp != NULL) {
      ...
      if (saved_pc) {
        ...
        if (dladdr(saved_pc, &info)) {
          frames.push_back(stack_frame(saved_pc, demangle(info.dli_sname) +
                                                     " in " + info.dli_fname));
        }
      }
    }
...
wxString get_backtrace(unsigned n) {
  ...
  while (it != frames.end()) {
    if (count >= n) {
      backtrace << (*it).instruction() << " : " << (*it).function()
                << std::endl;
      ++it;
    }
...
void print_backtrace(unsigned n) {
  ...
  // This is because the string is ansi anyway, and the conv classes are very
  // slow
